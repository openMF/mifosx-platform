...
while [[ ${#fineractmysql_pod} -eq 0 ]]; do
    fineractmysql_pod=$(microk8s kubectl get pods -l tier=fineractmysql --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')
done

fineractmysql_status=$(microk8s kubectl get pods ${fineractmysql_pod} --no-headers -o custom-columns=":status.phase")
